    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-white">
          <tr>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">ID Paciente</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">Nombre</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">Edad/Género</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">Contacto</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">Tipo de sangre</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {pacientes.map((paciente, index) => {
            const esUltimo = index >= pacientes.length - 3

            return (
              <tr key={paciente.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{paciente.id}</td>
                ...
                <td className="px-6 py-4 text-sm text-gray-500">{paciente.condicion}</td>
                ...
                <td className="px-6 py-4 text-sm text-gray-500">
                  <div className="flex space-x-3">
                    ...
                    <div className="relative">
                      <button
                        onClick={() =>
                          setMenuAbierto(menuAbierto === paciente.id ? null : paciente.id)
                        }
                      ...
                      {menuAbierto === paciente.id && (
                        <div
                          className={`absolute right-0 ${
                            esUltimo ? "bottom-full mb-2" : "top-full mt-2"
                          } w-48 bg-white rounded-md shadow-lg z-10 border`}
                        >
                          <div className="py-1">
